                          ...
                          <table style="width:100%;">
                                <tr  v-for="(item,subindex) in order.skuList" :key="subindex">
                                  ...
                                  <td class="t-column" :rowspan="order.skuList.length" v-if="subindex==0">
                                      <div>
                                        <img :src="order.userinfo.avatar|avatar" alt="" style="width:45px;height:45px;">
                                  ...
                                  <td class="t-column" style="width:336px;text-align:right;" :rowspan="order.skuList.length" v-if="subindex==0">
                                    ...
                                    <el-button size="mini" type="success" @click="sendProd(order)" v-if="order.orderStatus>=2 && order.orderStatus<5 && order.refundstatus==0 && order.shipType==0">发货</el-button>
                                    ...
                                    <el-button size="mini" type="primary" @click="recvProd(order)" v-if="order.orderStatus>=2 && order.orderStatus<=5 && order.refundstatus!=1 && order.shipType!=0">确认收货</el-button>
...
export default {
    ...
    methods: {
        ...
        getOrderList(keyword){
            ...
            if(this.$route.query.groundbookingid){
                params.actionid = this.$route.query.groundbookingid;
            }
            params.astype=3;
            this.loading=true;
            this.orderList = [];
            // this.req.post('/assemble/records',params).then(res=>{
            this.req.post('assemble/orderlist',params).then(res=>{
                this.loading=false;
                if(res.data.code=='SUCCESS' && res.data.sub_code=='SUCCESS'){
                    ...
                    //     item.orderList.forEach(sitem=>{
                    //         this.orderList.push(sitem);
                    //     })
                    // })
                }else{
        ...
        detail(item){
            ...
            this.$router.push({path:'/gmallorderdetail'});
        },
        exportOut() {
            /* 从表生成工作簿对象 */

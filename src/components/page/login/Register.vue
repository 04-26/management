      ...
      >
        ...
        <div style>
          <el-form-item label-width="0" prop="type">
            <el-checkbox v-model="ruleForm.type" label="注册即代表同意《用户使用协议》" name="type"></el-checkbox>
          </el-form-item>
          <p style="padding-left:15px;color:#00d1b2">
  ...
  data: function() {
    ...
    return {
      ...
      rules: {
        ...
        type: [
          // 密码验证改造
          { required: true, message: "请确认已阅读用户协议", trigger: "blur" } // 自定义验证规则
        ],
        // 确认密码验证
        checkPass: [
    ...
    submitForm() {
      this.$refs.ruleForm.validate(valid => {
        ...
        if (valid) {
          // 收集账号和密码
          const params = {
            username: this.ruleForm.telphone,

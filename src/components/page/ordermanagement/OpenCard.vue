<template>
    <div>
        ...
        <div class="prodlist">
            <el-card :body-style="{ padding: '0px' }" v-for="(item, index) in prodList" :key="index" class="prod-card"  style="cursor:pointer;">
                <img :src="$imgurl()+item.imageUrl" class="image" @click.stop="detail(item)">
                <div style="padding: 10px;" @click.stop="detail(item)">
                    ...
                    <el-row :gutter="10">
                        ...
                        <el-col :span="12" style="text-align:right;">
                            <span style="color:#ee5002;;font-size:14px;color:#00D1B2;cursor:pointer;" @click.stop="$emit('sel-massage',item)">选择</span>
                        </el-col>
                    </el-row>
                </div>
            </el-card>
...
export default {
    ...
    methods: {
        detail(item){
            console.log(1)
            this.$router.push({path:"/carddetail",query:{cardId:item.id}});
            this.$emit('card-detail',item);
        },
        tabChange(tab){
        ...
        search(val){
            this.$emit('search',this.massageTitle);
        },
        selmassgeType(id){
            this.typeId=id;
            this.$emit('tab',id);
